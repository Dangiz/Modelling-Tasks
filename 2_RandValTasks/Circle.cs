...
namespace RandValTasks
{
    ...
    public class Circle
    {
        ...
        {
            if (Point.GetDistance(a.Center, b.Center) >= a.Radius + b.Radius)
                return 0;
            ...
            for (int i=0;i<count;i++)
            {
                ...
                if (Point.GetDistance(tempPoint, a.Center) <= a.Radius && Point.GetDistance(tempPoint, b.Center) <= b.Radius)
                    intersectionHitCount++;
            }
            return (double)intersectionHitCount/count*rectArea;
        }

        private static Point GetRectangleRandomPoint(double left,double up,double right,double down,Random random)
        {
            
            return new Point(left + random.NextDouble() * (right - left), down + random.NextDouble() * (up - down));
        }
    }
}
